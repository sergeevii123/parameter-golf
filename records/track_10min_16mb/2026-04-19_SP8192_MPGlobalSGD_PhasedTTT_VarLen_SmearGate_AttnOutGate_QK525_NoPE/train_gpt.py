...
class Hyperparameters:
    ...
    nope = bool(int(os.environ.get("NOPE", "1")))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        gate_attn_out=False, gate_attn_src="proj", gate_width=12, nope=False,
    ):
        super().__init__()
        if dim % num_heads != 0:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            ...
            gate_attn_out=gate_attn_out, gate_attn_src=gate_attn_src, gate_width=gate_width, nope=nope,
        )
...
class GPT(nn.Module):
    ...
    def _block_with_lora(self, block, x, x0, lora, slot, q_w, k_w, v_w, out_w, up_w, down_w):
        ...
        if attn.gate_attn_out:
            gate_in = n[..., : attn.gate_width].contiguous()
