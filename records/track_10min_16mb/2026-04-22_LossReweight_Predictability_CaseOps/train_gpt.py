...
class Hyperparameters:
    ...
    # Loss reweighting by per-token predictability. alpha=0 -> standard CE.
    # alpha>0: weight_t = (nll_t / batch_mean_nll)^alpha, upweight hard tokens.
    # Weights are detached, clipped to [clip_lo, clip_hi], and renormalized so
    # the effective batch loss scale stays comparable to alpha=0.
    ...
    # Warmup: ramp alpha from 0 to target over this many steps. Avoids early
    # collapse onto noisy high-NLL tokens before the model has a baseline.
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Loss reweighting state. _train_step is updated externally each step.
        self.loss_reweight_alpha = h.loss_reweight_alpha
        self.loss_reweight_clip_lo = h.loss_reweight_clip_lo
        self.loss_reweight_clip_hi = h.loss_reweight_clip_hi
        self.loss_reweight_warmup_steps = h.loss_reweight_warmup_steps
    ...
    def forward(self, input_ids, target_ids, cu_seqlens=None, max_seqlen=0):
        ...
        # Predictability-weighted CE: weight_t = (nll_t / batch_mean_nll)^a_eff,
        # detached, clipped, then renormalized so mean(weight) == 1 (keeps loss
        # scale stable across alpha values for the optimizer). a_eff ramps from
        # 0 to alpha over warmup_steps so early training matches alpha=0.
        nll = F.cross_entropy(
            logits.reshape(-1, logits.size(-1)).float(),
            target_ids.reshape(-1),
            reduction="none",
        )
        warm_f = float(max(self.loss_reweight_warmup_steps, 1))
        ramp = torch.clamp(self._train_step.float() / warm_f, max=1.0)
        a_eff = self.loss_reweight_alpha * ramp
        with torch.no_grad():
            nll_d = nll.detach()
            mean_nll = nll_d.mean().clamp(min=1e-8)
            ratio = (nll_d / mean_nll).clamp(self.loss_reweight_clip_lo, self.loss_reweight_clip_hi)
            w = ratio.pow(a_eff)
...
def train_model(h, device, val_data):
    ...
    def step_fn(step, lr_scale):
        ...
        if hasattr(base_model, "_train_step"):
            base_model._train_step.fill_(step)
