...
class Hyperparameters:
    ...
    # Unigram prior on output logits. Adds a (vocab_size,) bias initialized to
    # log p(token) computed from the first N training shards. Bias is a fixed
    # buffer (non-trainable in v1). Disabled = bit-identical to the base.
    unigram_prior_enabled = bool(int(os.environ.get("UNIGRAM_PRIOR_ENABLED", "0")))
    unigram_prior_shards = int(os.environ.get("UNIGRAM_PRIOR_SHARDS", 1))
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Unigram prior bias on output logits. Fixed buffer (non-trainable in v1)
        # added inside _project_logits so all three forward paths (forward,
        # forward_logits, forward_ttt) inherit it. Pre-softcap → fused CE kernel
        # sees it correctly. Init zeros; populated from data in train_model().
        self.unigram_prior_enabled = bool(h.unigram_prior_enabled)
        if self.unigram_prior_enabled:
            self.register_buffer(
                "unigram_logits",
                torch.zeros(h.vocab_size, dtype=torch.float32),
                persistent=True,
            )
        self._init_weights()
    ...
    def _project_logits(self, hidden):
        if self.tie_embeddings:
            proj = F.linear(hidden, self.tok_emb.weight)
        else:
            proj = self.lm_head(hidden)
        if self.unigram_prior_enabled:
            proj = proj + self.unigram_logits.to(dtype=proj.dtype)
        return proj
...
def _compute_unigram_logits(h, device):
    files = sorted(glob.glob(h.train_files))[: max(h.unigram_prior_shards, 1)]
    if not files:
        raise FileNotFoundError(f"No train shards for unigram prior: {h.train_files}")
    counts = torch.zeros(h.vocab_size, dtype=torch.float64)
    for f in files:
        tok = load_data_shard(Path(f)).long()
        counts += torch.bincount(tok, minlength=h.vocab_size).double()
    counts.add_(1.0)  # Laplace smoothing for any zero-count tokens
    log_p = (counts.log() - counts.sum().log()).float()
    log(
        f"unigram_prior:shards={len(files)} tokens={int(counts.sum().item())} "
        f"log_p_range=[{log_p.min().item():.3f},{log_p.max().item():.3f}]"
    )
...
def train_model(h, device, val_data):
    ...
    if h.unigram_prior_enabled:
        base_model.unigram_logits.data.copy_(
            _compute_unigram_logits(h, device).to(base_model.unigram_logits.dtype)
        )
