...
# Dev-mode override: disable torch.compile (GB10 Blackwell hits Triton shared-mem limits
# on some compiled kernels). Submission on H100 leaves this off for full JIT perf.
if os.environ.get("DISABLE_COMPILE", "0") == "1":
    _orig_compile = torch.compile
    def _noop_compile(*args, **kw):
        if args and callable(args[0]):
            return args[0]
        return lambda f: f
    torch.compile = _noop_compile
# Attention backend resolution: prefer FA3 (H100 native), fall back to FA2 (Spark GB10)
# and finally to PyTorch SDPA. Submission runs on H100 where FA3 is available,
# so the first branch is the intended production path. The fallbacks exist only
# to enable dev/smoke testing on Blackwell (SM12.1) where FA3 kernels lack SM120/121
# support (see Dao-AILab/flash-attention#1969).
_FA_BACKEND = "fa3"
# Force FA2 when on Blackwell SM12.x (FA3 imports but kernels lack SM12.x).
# FORCE_FA2=1 override or auto-detect cc==12.
_FORCE_FA2 = os.environ.get("FORCE_FA2", "0") == "1"
if not _FORCE_FA2 and torch.cuda.is_available():
    _cc = torch.cuda.get_device_capability(0)
    if _cc[0] == 12:
        _FORCE_FA2 = True
try:
    if _FORCE_FA2:
        raise ImportError("FORCE_FA2 path on Blackwell SM12.x")
    ...
    try:
        ...
        _fa2_func = _fa2.flash_attn_func
        _fa2_varlen = _fa2.flash_attn_varlen_func
        def flash_attn_3_func(q, k, v, causal=False, **kw):
            r = _fa2_func(q, k, v, causal=causal, **kw)
            return r[0] if isinstance(r, tuple) else r
        def flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k,
                                   max_seqlen_q, max_seqlen_k, causal=False,
                                   window_size=(-1, -1), **kw):
            r = _fa2_varlen(q, k, v, cu_seqlens_q, cu_seqlens_k,
                            max_seqlen_q, max_seqlen_k, causal=causal,
                            window_size=window_size, **kw)
            return r[0] if isinstance(r, tuple) else r
        _FA_BACKEND = "fa2"
    except Exception:
        import torch.nn.functional as _F_sdpa
        def flash_attn_3_func(q, k, v, causal=False, **kw):
            # (B, T, H, D) -> (B, H, T, D) for SDPA, then back
            q_, k_, v_ = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
            out = _F_sdpa.scaled_dot_product_attention(q_, k_, v_, is_causal=causal)
            return out.transpose(1, 2)
        def flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k,
                                   max_seqlen_q, max_seqlen_k, causal=False,
                                   window_size=(-1, -1), **kw):
            # Emulate varlen by running SDPA per-sequence. Slow but correct for smoke.
            outs = []
            for i in range(len(cu_seqlens_q) - 1):
                s_q, e_q = int(cu_seqlens_q[i]), int(cu_seqlens_q[i + 1])
                s_k, e_k = int(cu_seqlens_k[i]), int(cu_seqlens_k[i + 1])
                q_i = q[s_q:e_q].transpose(0, 1).unsqueeze(0)
                k_i = k[s_k:e_k].transpose(0, 1).unsqueeze(0)
                v_i = v[s_k:e_k].transpose(0, 1).unsqueeze(0)
                o_i = _F_sdpa.scaled_dot_product_attention(q_i, k_i, v_i, is_causal=causal)
                outs.append(o_i.squeeze(0).transpose(0, 1))
...
class Hyperparameters:
    ...
    wd_taper_start_frac = float(os.environ.get("WD_TAPER_START_FRAC", 1.0))  # 1.0 = no taper
    wd_taper_final_mult = float(os.environ.get("WD_TAPER_FINAL_MULT", 1.0))  # 1.0 = no taper
    ...
    loss_reweight_alpha = float(os.environ.get("LOSS_REWEIGHT_ALPHA", "0.0"))
    loss_reweight_clip_lo = float(os.environ.get("LOSS_REWEIGHT_CLIP_LO", "0.1"))
    loss_reweight_clip_hi = float(os.environ.get("LOSS_REWEIGHT_CLIP_HI", "10.0"))
    loss_reweight_warmup_steps = int(os.environ.get("LOSS_REWEIGHT_WARMUP_STEPS", "500"))
...
def linear_leaky_relu_square(a, b, aux=None):
    ...
    # Blackwell (SM12.x, e.g. GB10) has ~101KB shared memory per SM vs Hopper ~228KB.
    # Reduce block sizes and num_stages to fit the fused kernel. Production on H100
    # uses full 128/256/64 with 4 stages.
    _cc = torch.cuda.get_device_capability(a.device)
    _is_blackwell_small = _cc[0] == 12
    if _is_blackwell_small:
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.loss_reweight_alpha = float(h.loss_reweight_alpha)
        self.loss_reweight_clip_lo = float(h.loss_reweight_clip_lo)
        self.loss_reweight_clip_hi = float(h.loss_reweight_clip_hi)
        self.loss_reweight_warmup_steps = int(h.loss_reweight_warmup_steps)
    ...
    def forward(self, input_ids, target_ids, cu_seqlens=None, max_seqlen=0):
        ...
        if self.loss_reweight_alpha == 0.0:
        ...
        warmup = max(1, int(self.loss_reweight_warmup_steps))
        step = self._train_step.to(dtype=nll.dtype)
        ramp = torch.clamp(step / warmup, max=1.0)
        a_eff = float(self.loss_reweight_alpha) * ramp
        nll_d = nll.detach()
        ratio = (nll_d / nll_d.mean().clamp_min(1e-8)).clamp(
            self.loss_reweight_clip_lo, self.loss_reweight_clip_hi
        )
        w = ratio.pow(a_eff)
...
# ===== MP-SGD phased TTT port from pr1626 =====
...
def train_model(h, device, val_data):
    ...
    def step_fn(step, lr_scale):
        if hasattr(base_model, "_train_step"):
            base_model._train_step.fill_(int(step))
...
def train_and_eval(h, device):
    ...
    if h.ttt_enabled:
        ...
        _ttt_eval_fn = (
            eval_val_ttt_phased if h.phased_ttt_enabled else eval_val_ttt_lora
        )
        ttt_val_loss, ttt_val_bpb = _ttt_eval_fn(
            h, ttt_model, device, val_data, forward_ttt_train=fwd_ttt_compiled
        )
        torch.cuda.synchronize()
        ttt_eval_elapsed = time.perf_counter() - t_ttt
        log(
            f"quantized_ttt_{'phased' if h.phased_ttt_enabled else 'lora'} val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f} eval_time:{1e3*ttt_eval_elapsed:.0f}ms"
        )
