...
class Hyperparameters:
    ...
    asymmetric_loop = bool(int(os.environ.get("ASYMMETRIC_LOOP", "1")))
    asymmetric_shrink_per_pass = int(os.environ.get("ASYMMETRIC_SHRINK_PER_PASS", 1))
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        if h.num_loops > 0:
            ...
            if h.asymmetric_loop:
                pass_end = h.loop_end
                for _ in range(h.num_loops + 1):
                    all_indices.extend(range(h.loop_start, pass_end + 1))
                    pass_end = max(h.loop_start + 1, pass_end - h.asymmetric_shrink_per_pass)
            else:
