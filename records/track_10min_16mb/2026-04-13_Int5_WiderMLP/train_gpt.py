"""
Int5 GPTQ + Wider MLP experiment.

Based on the SOTA (1.0810 BPB) from 2026-04-09_SP8192_3LayerRecur_ParResid_QK525_LegalTTT.
Changes from SOTA:
  - matrix_bits: 6 -> 5  (frees ~4MB of parameter budget)
  - mlp_mult: 4 -> 5     (uses freed budget: hidden 2048 -> 2560)
  - matrix_clip_sigmas: 12.85 -> 9.0  (tuned for int5's narrower range)
"""
...
class Hyperparameters:
    ...
    mlp_mult = float(os.environ.get("MLP_MULT", 5.0))  # <-- was 4.0 in SOTA
    ...
    matrix_bits = int(os.environ.get("MATRIX_BITS", 5))             # <-- was 6 in SOTA
    embed_bits = int(os.environ.get("EMBED_BITS", 8))
    matrix_clip_sigmas = float(os.environ.get("MATRIX_CLIP_SIGMAS", 9.0))  # <-- was 12.85 in SOTA
...
class Block(nn.Module):
    def __init__(
        self, dim, num_heads, num_kv_heads, mlp_mult, rope_base,
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Parallel residuals from layer N
...
class Muon(torch.optim.Optimizer):
    def __init__(
        self, params, lr, momentum, backend_steps,
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
    ...
    for ci in range(num_chunks):
        ...
        # Score phase: evaluate all windows in this chunk under no_grad
        with torch.no_grad():
            for bi in range(0, len(my_windows), batch_seqs):
                batch_ws = my_windows[bi : bi + batch_seqs]
                bsz = len(batch_ws)
        ...
        # Update phase: train on scored chunk tokens
